#!/usr/bin/env bash
# agentstash installer
# curl -fsSL https://agentstash.io/install.sh | sh
set -euo pipefail
...
echo ""
info "  agentstash installer"
dim "Encrypted backups for AI coding agents"
echo ""
...
if [ "$NODE_VERSION" -lt 18 ]; then
  fail "Node.js $NODE_VERSION found, but agentstash requires Node 18+. Upgrade: https://nodejs.org"
fi
...
if command -v npm &>/dev/null; then
  PM="npm"
  PM_INSTALL="npm install -g agentstash"
elif command -v pnpm &>/dev/null; then
  PM="pnpm"
  PM_INSTALL="pnpm add -g agentstash"
elif command -v bun &>/dev/null; then
  PM="bun"
  PM_INSTALL="bun add -g agentstash"
else
...
echo ""
info "  Installing agentstash..."
dim "$PM_INSTALL"
echo ""

if $PM_INSTALL; then
  echo ""
  ok "agentstash installed"
else
  echo ""
...
if ! command -v agentstash &>/dev/null; then
  warn "agentstash installed but not in PATH"
  dim "You may need to restart your shell or add the global bin to your PATH"
  echo ""
  exit 0
fi

VERSION="$(agentstash --version 2>/dev/null || echo "unknown")"
ok "agentstash $VERSION ready"
...
echo ""
printf "  ${BOLD}agentstash setup${RESET}\n"
echo ""
dim "This will:"
