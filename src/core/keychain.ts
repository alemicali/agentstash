...
import { getPlatform, getAgentstashDir } from "../utils/platform.js";
import { log } from "../utils/logger.js";

const execFileAsync = promisify(execFile);

const SERVICE_NAME = "agentstash";
const ACCOUNT_NAME = "backup-passphrase";

function getPassphraseFilePath(): string {
  return join(getAgentstashDir(), "passphrase");
}
 ...
 * Falls back to ~/.agentstash/passphrase (mode 600) if keychain is unavailable.
 *
 * - macOS: Keychain Access via `security` CLI
 * - Linux: GNOME Keyring / KDE Wallet via `secret-tool` CLI
 * - Fallback: ~/.agentstash/passphrase file
 */
  ...
  try {
    ...
    if (platform === "linux") {
      const child = execFile("secret-tool", [
        "store",
        "--label", "agentstash backup passphrase",
 ...
 * Falls back to ~/.agentstash/passphrase if keychain is unavailable.
 */
