...
import { getServiceManager, getAgentstashDir } from "../utils/platform.js";
import { ensureDir, pathExists } from "../utils/fs.js";
import { requireConfig } from "../core/config.js";

const execFileAsync = promisify(execFile);

const LAUNCHD_LABEL = "dev.agentstash.backup";
const SYSTEMD_SERVICE = "agentstash-backup";

/**
 * Resolve the absolute path to the agentstash binary.
 * Prefers the globally installed `agentstash` in PATH (survives nvm switches),
 * falls back to process.argv[1] (the current script).
 */
async function resolveAgentstashBin(): Promise<string> {
  try {
    const { stdout } = await execFileAsync("which", ["agentstash"]);
  ...
  return process.argv[1] ?? "agentstash";
}
...
function buildLaunchdPlist(intervalMinutes: number, agentstashBin: string): string {
...
<dict>
  ...
  <string>${getAgentstashDir()}/daemon.log</string>
...
function buildSystemdService(agentstashBin: string): string {
  return `[Unit]
Description=agentstash backup for AI coding agents
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=${agentstashBin} backup
...
function buildSystemdTimer(intervalMinutes: number): string {
  return `[Unit]
Description=agentstash backup timer
...
export async function daemonInstall(): Promise<void> {
  ...
  const agentstashBin = await resolveAgentstashBin();

  log.debug(`Daemon will use binary: ${agentstashBin}`);

  if (sm === "launchd") {
    const plistPath = getLaunchdPlistPath();
    await ensureDir(join(plistPath, ".."));
    await writeFile(plistPath, buildLaunchdPlist(interval, agentstashBin), "utf-8");
    ...
    await writeFile(servicePath, buildSystemdService(agentstashBin), "utf-8");
