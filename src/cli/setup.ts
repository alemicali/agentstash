...
import { getInstalledAgents, getAllAgents } from "../core/agents/registry.js";
...
import {
  ...
  type AgentstashConfig,
  type AgentConfig,
  type StorageConfig,
} from "../core/config.js";

export async function setupCommand(): Promise<void> {
  log.header("agentstash setup");
  ...
  if (existing) {
    const { overwrite } = await prompts({
      type: "confirm",
      name: "overwrite",
      message: "agentstash is already configured. Reconfigure?",
      initial: false,
    });
    if (!overwrite) {
  ...
  log.info(chalk.dim("[1/5]") + " Detecting AI coding agents...");
  log.blank();
  ...
  if (installed.length > 0) {
    log.info(`Found ${installed.length} installed agent${installed.length > 1 ? "s" : ""}:`);
    for (const agent of installed) {
      const dirs = agent.dataDirs.map((d) => d.path).join(", ");
      log.info(`  ${chalk.green("*")} ${agent.displayName} ${chalk.dim(`(${dirs})`)}`);
    }
  } else {
    log.warn("No known agents detected.");
    log.info(chalk.dim(`Checked: ${all.map((a) => a.displayName).join(", ")}`));
  }

  log.blank();

  // Multi-select from installed agents (with option to add undetected ones)
  const agentChoices = all.map((agent) => {
    const isInstalled = installed.some((i) => i.id === agent.id);
    return {
      title: `${agent.displayName}${isInstalled ? chalk.green(" (detected)") : chalk.dim(" (not found)")}`,
      value: agent.id,
      selected: isInstalled,
    };
  });

  const { selectedAgentIds } = await prompts({
    type: "multiselect",
    name: "selectedAgentIds",
    message: "Select agents to back up:",
  ...
  if (!selectedAgentIds || selectedAgentIds.length === 0) {
    log.error("No agents selected. At least one agent is required.");
    process.exit(1);
  }

  // Build agent configs, confirm data directories for each
  const agentConfigs: AgentConfig[] = [];

  for (const agentId of selectedAgentIds) {
    const agentDef = all.find((a) => a.id === agentId);
    if (!agentDef) continue;

    const dirs = agentDef.dataDirs.map((d) => d.path).join(", ");
    log.blank();
    log.info(`${chalk.bold(agentDef.displayName)} data directories: ${chalk.dim(dirs)}`);

    const { confirmed } = await prompts({
      type: "confirm",
      name: "confirmed",
      message: `Use default directories for ${agentDef.displayName}?`,
      initial: true,
    });

    if (confirmed) {
      agentConfigs.push({ id: agentId, enabled: true });
    } else {
      const { customDir } = await prompts({
        type: "text",
        name: "customDir",
        message: `Custom data directory for ${agentDef.displayName}:`,
      });

      if (customDir) {
        agentConfigs.push({ id: agentId, dataDir: customDir, enabled: true });
      } else {
        agentConfigs.push({ id: agentId, enabled: true });
      }
    }
  ...
  if (saved && keychainOk) {
    ...
    log.success("Passphrase saved to ~/.agentstash/passphrase");
  } else {
    log.warn("Could not save passphrase. Set AGENTSTASH_PASSPHRASE env var to avoid prompts.");
  }
  ...
  if (saved) {
    if (keychainOk) {
      ...
      log.info(chalk.dim("Passphrase is stored in ~/.agentstash/passphrase. Backups will run without prompts."));
    }
  } else {
    log.info(chalk.yellow("Important: store your passphrase safely!"));
    log.info(chalk.dim("Set AGENTSTASH_PASSPHRASE env var, or pass --passphrase to each command."));
  }
  log.blank();

  // ── Summary ──────────────────────────────────────────────────────────────

  log.info(`Configured ${agentConfigs.length} agent${agentConfigs.length > 1 ? "s" : ""} for backup:`);
  for (const ac of agentConfigs) {
    const def = getAllAgents().find((a) => a.id === ac.id);
    const name = def?.displayName ?? ac.id;
    const dir = ac.dataDir ?? def?.dataDirs.map((d) => d.path).join(", ") ?? "default";
    log.info(`  ${chalk.green("*")} ${name} ${chalk.dim(`(${dir})`)}`);
  }
  log.blank();
  ...
  if (interval > 0) {
    ...
    if (installDaemon) {
      try {
        ...
        log.info(chalk.dim(`Run manually later: agentstash daemon install`));
  ...
  if (interval > 0) {
    ...
    log.info(chalk.dim("Run `agentstash backup` when you want to back up."));
  }
  log.blank();
}
