...
// Import agents module to trigger registration of all agent definitions
...
program
  .name("agentstash")
  .description("Encrypted backups for AI coding agents")
  .version(version)
...
program
  .command("setup")
  .description("Configure agentstash (storage, encryption, schedule)")
  .action(async () => {
...
program
  ...
  .option("--agent <id>", "Backup only a specific agent")
  .option("--only <category>", "Backup only: config, secrets, workspace, sessions, memory, etc.")
...
program
  ...
  .option("-p, --passphrase <passphrase>", "Encryption passphrase")
  .option("--agent <id>", "Agent to restore (required if multiple agents configured)")
  .option("--only <category>", "Restore only: config, secrets, workspace, sessions, memory, etc.")
...
program
  ...
  .option("-p, --passphrase <passphrase>", "Encryption passphrase")
  .option("--agent <id>", "Filter snapshots by agent")
  .action(async (opts) => {
...
program
  ...
  .option("-p, --passphrase <passphrase>", "Encryption passphrase")
  .option("--agent <id>", "Apply retention only to a specific agent's snapshots")
  .action(async (opts) => {
