...
- Auto-detect R2 jurisdiction (EU vs default) — no more manual selection
- Fixed `prompts` select returning numeric index instead of string value for jurisdiction
...
- Passphrase file fallback (`~/.agentstash/passphrase`, mode 600) when system keychain is unavailable
...
- Fixed daemon binary path resolution (uses `which agentstash` instead of `process.argv`)
...
- Health checks via `agentstash doctor`
