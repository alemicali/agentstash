...
vi.mock("../src/utils/platform.js", () => ({
  ...
  getAgentstashDir: () => "/home/user/.agentstash",
}));
...
describe("daemon", () => {
  ...
  describe("daemonInstall", () => {
    it("creates systemd service and timer files on Linux", async () => {
      ...
      expect(serviceContent).toContain("agentstash backup");
    ...
    it("creates launchd plist on macOS", async () => {
      ...
      expect(plistContent).toContain("dev.agentstash.backup");
      expect(plistContent).toContain("agentstash");
