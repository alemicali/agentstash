...
vi.mock("../src/utils/platform.js", () => ({
  getResticBinaryPath: () => "/mock/.agentstash/bin/restic",
  getAgentstashBinDir: () => "/mock/.agentstash/bin",
  getResticAssetName: (v: string) => `restic_${v}_linux_amd64.bz2`,
  getTempDir: () => "/tmp/agentstash",
  getPlatform: () => "linux",
}));
...
describe("restic-installer", () => {
  ...
  describe("ensureRestic", () => {
    it("returns early if already installed", async () => {
      ...
      expect(result).toBe("/mock/.agentstash/bin/restic");
    });
  });
});
