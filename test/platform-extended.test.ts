...
describe("platform utilities", () => {
  ...
  describe("directory paths", () => {
    it("getAgentstashDir returns path in home directory", () => {
      expect(getAgentstashDir()).toBe(join(homedir(), ".agentstash"));
    });

    it("getAgentstashBinDir is inside agentstash dir", () => {
      expect(getAgentstashBinDir()).toBe(join(homedir(), ".agentstash", "bin"));
    });

    it("getAgentstashConfigPath is config.json inside agentstash dir", () => {
      expect(getAgentstashConfigPath()).toBe(
        join(homedir(), ".agentstash", "config.json"),
      );
    });
    ...
    it("getTempDir is in system temp", () => {
      expect(getTempDir()).toBe(join(tmpdir(), "agentstash"));
    });
  });
  ...
  describe("getResticBinaryPath", () => {
    it("is inside bin directory", () => {
      ...
      expect(path.startsWith(getAgentstashBinDir())).toBe(true);
    });
  });
